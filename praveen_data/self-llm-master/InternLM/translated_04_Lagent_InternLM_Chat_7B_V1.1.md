...
Select the same `AutoDL` image environment as the first `InternLM`, and run the following command to install dependencies. If the previous `InternLM-Chat-7B` has already configured the environment, there is no need to install it again.
...
Create a new `download.py` in the `/root/autodl-tmp` path file and enter the following content into it, and run `python /root/autodl-tmp/download.py` to download. The model size is 14 GB, and it takes about 10 to 20 minutes to download the model.
...
First, we run the following command to open the academic acceleration `clone` `lagent` repository, and install `Lagent` through `pip install -e .` source code

```shell
source /etc/network_turbo
cd /root/autodl-tmp
git clone https://github.com/InternLM/lagent.gitgit checkout 511b03889010c4811b1701abb153e02b8e94fb5e # Try to keep it consistent with the tutorial commit version
cd lagent
pip install -e . # Source code installation
unset http_proxy && unset https_proxy
```

## Modify the code

Since there are many places to modify the code, just replace the content of `/root/autodl-tmp/lagent/examples/react_web_demo.py` with the following code
...
Use the same method to map the port to the local. After running successfully, we select the `InternLM` model on the `Web` page. After waiting for the model to load, enter the math problem. Given `2x+3=10`, find `x` , at this time `InternLM-Chat-7B-V1.1` The model understands the problem and generates `Python` code to solve the problem. `Lagent` schedules it to be sent to the `Python` code interpreter to find the solution to the problem.
