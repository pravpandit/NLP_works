# Additional Experiment to Classify Sentiment of 50k IMDB Movie Reviews
...
These codes use 50k movie reviews from IMDb to predict whether a movie review is positive or negative. ([dataset](https://ai.stanford.edu/~amaas/data/sentiment/)) 

Run the following code to create `train.csv`, `validation.csv`, and `test.csv` datasets:
...
The 124M GPT-2 model used in the main chapter, starting from the pre-trained weights, train only the last Transformer block plus the output layer:
...
A 66M parameter encoder model [DistilBERT](https://arxiv.org/abs/1910.01108) (distilled from a 340M parameter BERT model), starting from pre-trained weights, training only the last Transformer block and output layer:
...
Ep 2 (SEp 004450): Train loss 0.204, Val loss 0.295
...
...
Training completioned in 16.70 minutes.
...
A 355M parameter encoder model [RoBERTa](https://arxiv.org/abs/1907.11692) , starting from pre-trained weights, training only the last Transformer block and output layer:
